...
class FeatureChoiceView(View):
    def get(self, request, *args, **kwargs):
        ...
        for item in feature_key_qs:
            res_string += option.format(
                value=item.feature_name, option_name=item.feature_name
            )
        html_select = html_select.format(result=res_string)
        return JsonResponse(
            {"result": html_select, "value": int(request.GET.get("category_id"))}
        )
...
class CreateFeatureView(View):
    def get(self, request, *args, **kwargs):
        ...
        feature = CategoryFeature.objects.get(
            category=category, feature_name=feature_name
        )
...
class SearchProductAjaxView(View):
    def get(self, request, *args, **kwargs):
        ...
        category = Category.objects.get(id=int(category_id))
        products = list(
            Product.objects.filter(category=category, title__icontains=query).values()
        )
...
class AttachNewFeatureToProduct(View):
    def get(self, request, *args, **kwargs):
        ...
        existing_features = list(
            set([item.feature.feature_name for item in product.features.all()])
        )
        ...
        for item in category_features:
            res_string += option.format(
                value=item.category.id, option_name=item.feature_name
            )
        html_select = html_select.format(result=res_string)
...
class ProductFeatureChoicesAjaxView(View):
    def get(self, request, *args, **kwargs):
        ...
        validators_qs = FeatureValidator.objects.filter(
            category=category, feature_key=feature_key
        )
        option = '<option value="{value}">{option_name}</option>'
        html_select = """
        ...
        for item in validators_qs:
            res_string += option.format(
                value=item.id, option_name=item.valid_feature_value
            )
        html_select = html_select.format(result=res_string)
        return JsonResponse({"features": html_select})
...
class CreateNewProductFeatureAjaxView(View):
    def get(self, request, *args, **kwargs):
        ...
        feature = ProductFeature.objects.create(
            feature=category_feature, product=product, value=value
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        for item in features_values_qs:
            fv_qs = FeatureValidator.objects.filter(
                category=item.product.category, feature_key=item.feature
            ).values()
            for fv in fv_qs:
                if fv["valid_feature_value"] == item.value:
                    pass
                else:
                    select_different_values_dict[fv["feature_key_id"]].append(
                        fv["valid_feature_value"]
                    )
            ...
            body_feature_field = body_feature_field.format(
                feature_field.format(
                    id=item.feature.id, value=item.feature.feature_name
                )
            )
            current_feature_value_mid_res = ""
            for item_ in select_different_values_dict[item.feature.id]:
                current_feature_value_mid_res += option.format(
                    value=item.feature.id, option_name=item_
                )
            body_feature_field_value = body_feature_field_value.format(
                select_values.format(
                    item.feature.id, result=current_feature_value_mid_res
                )
            )
...
class UpdateProductFeaturesAjaxView(View):
    def post(self, request, *args, **kwargs):
        ...
        data_for_update = [
            {"feature_name": name, "current_value": curr_val, "new_value": new_val}
            for name, curr_val, new_val in zip(
                features_names, features_current_values, new_feature_values
            )
        ]
        ...
        for item in product.features.all():
            for item_for_update in data_for_update:
                if item.feature.feature_name == item_for_update["feature_name"]:
                    if (
                        item.value != item_for_update["new_value"]
