...
class Category(models.Model):
    ...
    def get_fields_for_filter_in_template(self):
        return (
            CategoryFeature.objects.filter(category=self, use_in_filter=True)
            .prefetch_related("category")
            .values("feature_key", "feature_measure", "feature_name", "filter_type")
        )
...
class Product(models.Model):
    ...
    features = models.ManyToManyField(
        "specs.ProductFeature", blank=True, related_name="features_for_product"
    )
...
class Customer(models.Model):
    ...
    address = models.CharField(
        verbose_name="Адресс", max_length=255, null=True, blank=True
    )
...
class Order(models.Model):
    ...
    cart = models.ForeignKey(
        Cart, verbose_name="Корзина", on_delete=models.CASCADE, null=True, blank=True
    )
    address = models.CharField(
        verbose_name="Адресс доставки", max_length=1024, null=True, blank=True
    )
    ...
    created_at = models.DateTimeField(
        auto_now=True, verbose_name="Дата создание заказа"
    )
